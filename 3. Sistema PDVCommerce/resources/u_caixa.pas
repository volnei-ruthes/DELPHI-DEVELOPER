...
procedure Tfrm_caixa.FecharCaixa;
    ...
    //*******FALTA TERMINAR TODA ESSA ROTINA - ESTA INCOMPLETA AINDA*******//
...
begin
  ...
  registroCaixaPDV.valorQuebra := registroCaixaPDV.valorAbertura - registroCaixaPDV.valorVendido;
  ...
  with dm.FDQueryCaixa do begin
    ...
    SQL.Add('WHERE funcionario_operador = :pFuncOperador AND status = :pStatusAberto AND data_abertura = :pDataAbertura');
    ...
    ParamByName('pFuncOperador').Value                   := FuncionarioCaixaPDV.Id;
    ParamByName('pStatusAberto').Value                   := 'A';
    ParamByName('data_abertura').Value                   := StrToDate(FormatDateTime('dd/mm/yyyy',Now));
    ExecSQL;
  end;
